...
- CatBoost(GS parameters) with SMOTE data balancing technique, 
...
def blackbox_function(band_combination, hsi_image, gt, feature_importance, test_size = 0.2, random_state=42):
    ...
    # Create CatBoost classifier
    cbc = CatBoostClassifier(
        task_type='GPU',                     # Set to CPU, can be changed to 'GPU'
...
print(f"total band{len(new_indices_list_pavia_u+deleted_indices_list_pavia_u)}, Number of bands after reduction: {len(new_indices_list_pavia_u)}")
...
print(f"total band{len(new_indices_list_salinas+deleted_indices_list_salinas)}, Number of bands after reduction: {len(new_indices_list_salinas)}")
...
print(f"total band{len(new_indices_list_indian_pines+deleted_indices_list_indian_pines)}, Number of bands after reduction: {len(new_indices_list_indian_pines)}")
